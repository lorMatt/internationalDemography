...
### 2000
it00 <-
  read_excel("Dati/Italia - Ricostruzione intercensuaria della
            popolazione 1992-2001.xlsx",
  skip = 1
) |>
  select(1:2 | "2000") |>
  slice(1:303) |>
...
fecdf <- full_join(tftCitt, etaCitt) |>
  select(!Note) |>
  pivot_longer(
    cols = `Tasso di fecondità totale, madri italiane`:`Età media al parto, tutte le madri`,
    names_to = "var"
  ) |>
...
umPrev <- umPrev |>
  select(
    Anno, `Limite inferiore intervallo di confidenza al 90% (5° percentile)`,
    `Scenario mediano`,
